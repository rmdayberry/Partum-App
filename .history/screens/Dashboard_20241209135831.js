  ...
  useEffect(()=>{
    ...
    const fetchDailyTipForDay = async () => {
      try {
        ...
        const progress = await fetchPregnancyProgress(userId);
        const currentDay = Math.floor((progress / 100) * 280); // Convert progress % to day
        const tipData = await fetchDailyTip(currentDay);
        setDailyTip(tipData);
      } catch (error) {
