...
const SymptomChecker = () => {
  ...
  useEffect(() => {
    const fetchSymptoms = async () => {
      try {
        ...
        console.log("Fetched Symptoms:", data); // Debug log
        setSymptoms(data);
      } catch (error) {
          ...
          {symptom.specificCases.map((caseItem, caseIndex) => (
            <View key={caseIndex} style={styles.caseContainer}>
              <Text style={styles.caseTitle}>
                {caseItem.caseTitle[language]}
              </Text>
              <Text style={styles.caseContent}>
                {caseItem.details[language]}
              </Text>
            </View>
          ))}
        </View>
  ...
  const renderSymptomItem = ({ item }) => (
    <TouchableOpacity
      style={styles.item}
      onPress={() => {
        console.log("Selected symptom:", item); // Debug log
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        ...
        <Text style={styles.noDataText}>No symptoms available.</Text>
      )}
    </View>
  );
};
