...
const TabContent = ({heading, content, title, contentArray, subtitle2, details2, subtitle}) => {
  return (
    <View style={styles.container}>
      ...
      {contentArray &&
        contentArray.map((item, index) => (
          <View key={index} style={styles.section}>
            ...
            {item.details && <Text style={styles.content}>{item.details}</Text>}
          </View>
        ))}

      {/* Tips Section */}
      {subtitle2 && <Text style={styles.subheading}>{subtitle2}</Text>}
      {details2 &&
      ...
      {/* Call to Action */}
      {subtitle && <Text style={styles.callToAction}>{subtitle}</Text>}
    </View>
  );
};
