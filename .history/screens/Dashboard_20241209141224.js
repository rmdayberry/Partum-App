  ...
  useEffect(() => {
    const fetchProgressAndTips = async () => {
      try {
        ...
        if (isNaN(progress)) {
          console.error("Invalid progress value:", progressData);
          return;
        }

        // Calculate current week and day
        const week = Math.floor((progress / 100) * 40);
        const day = Math.floor((progress / 100) * 280);

        // Set valid week and day
        if (week >= 1 && week <= 40) setCurrentWeek(week);
        if (day >= 1 && day <= 280) setCurrentDay(day);
