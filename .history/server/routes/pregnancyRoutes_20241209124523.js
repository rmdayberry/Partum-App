...
const DailyTip = require("../models/DailyPregnancyTips");
...
//Fetch daily tip
router.get("/api/daily-tip/:day", async (req, res) => {
  try {
    const day = parseInt(req.params.day, 10);
    const tip = await DailyTip.findOne({ day });
    if (!tip) {
      return res.status(404).json({ message: "Daily tip not found" });
    }
    res.json(tip);
  } catch (error) {
