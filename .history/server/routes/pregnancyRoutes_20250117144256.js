...
// Endpoint to fetch weekly tips by userId (UPDATED)
router.get("/whatToExpectWeekly/:userId", async (req, res) => {
  try {
    ...
    if (!user.dueDate || !user.languagePreference) {
      return res.status(400).json({
        message:
          "Missing required user information (dueDate or languagePreference).",
      });
    }
    ...
    const tip =
      user.languagePreference === "Español" ? weeklyTip.tipEs : weeklyTip.tip;

    res.json({ week: currentWeek, tip });
  } catch (err) {
    console.error("Error in /whatToExpectWeekly:", err.message);
    res.status(500).json({ message: "Server error", error: err.message });
  }
});
